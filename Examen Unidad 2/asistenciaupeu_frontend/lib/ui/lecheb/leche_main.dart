...
class MainLecheB extends StatelessWidget {
  const MainLecheB({super.key});
...
class _LecheUIState extends State<LecheUI> {
            ...
            Padding(
              ...
              child: GestureDetector(
                onTap: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => LecheForm()),
                  ).then(onGoBack);
                },
  ...
  Widget _buildListView(BuildContext context, List<LecheModelo> leche) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 8.0),
      child: ListView.builder(
        itemBuilder: (context, index) {
          LecheModelo lechex = leche[index];
          return Padding(
                              ...
                              Container(
                                decoration: BoxDecoration(
                                    ...
                                    color: AppTheme.themeData.colorScheme
                                        .primaryContainer),
                                child: Text(
                                  //lechex.asistenciaxs.isEmpty? lechex.evaluar:lechex.asistenciaxs.first.horaReg,
                                  lechex.fecha,
                                          ...
                                          onPressed: () {
                                            showDialog(
                                              ...
                                              if (value.toString() ==
                                                  "Success") {
                                                print(lechex.id);
                                                /*Provider.of<LecheApi>(context,
                                                listen: false)
                                                .deleteLeche(TokenUtil.TOKEN,lechex.id)
  ...
  void exportAsistenciaToExcel(List<AsistenciapaxModelo> asistencia) {
    ...
    for (var row = 0; row < asistencia.length; row++) {
      ...
      sheetObject
          ...
          .value = asistenciax.Leche.nombreLeche;
      sheetObject
